...
let _viewer: Viewer;
let _roads: Route[];
let _roadToCartesian3Positions: { [index: number]: Cartesian3[] } = {};
let options: {
	color: string;
	create: Function;
	clear: Function;
...
export const example = (viewer: Viewer, gui: GUI) => {
	...
	const bridge = RouteFactory.createRouteModel(
		data,
		arr => {
			return new BigeMapUtils.Vector3(...arr);
		},
		BigeMapUtils.createRouteModelGeometryAttribute,
	...
	folder
		...
		.onChange(v => {
			_roads.forEach((road, i) => {
				const attributes = RouteFactory.createAttributes(
					_roadToCartesian3Positions[i],
					arr => {
						return new BigeMapUtils.Vector3(...arr);
					},
					BigeMapUtils.createRouteModelGeometryAttribute,
	...
	_handler.setInputAction((event: { position: Cartesian2 }) => {
		...
		if (cartesian) {
			if (cartesian3_positions.length > 0 && cartesian.equals(last)) {
				...
				const road = RouteFactory.createRouteModel(
