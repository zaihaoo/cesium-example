import { Cartesian3, Color, Math, Matrix3, Matrix4, Text3D, Transforms, Viewer } from 'cesium';
...
const createText3D = async () => {
	const typeface = await (BigeMapUtils as any).fontLoader('Assets/File/Typeface/KaiTi_Regular.json');
	...
	const fontGeo = (BigeMapUtils as any).createText3DGeometryAttribute('大连市', typeface);
	const fontGeo2 = (BigeMapUtils as any).createText3DGeometryAttribute('BOOM', typeface);

	text3d_1 = _viewer.scene.primitives.add(
		new Text3D({
	...
	text3d_2 = _viewer.scene.primitives.add(
		new Text3D({
...
export const example = async (viewer: Viewer, gui: GUI) => {
	...
	viewer.scene.preUpdate.addEventListener(function (_scene, time) {
		...
		Matrix4.multiplyByTranslation(
			_modelMatrix2,
			new Cartesian3(0, window.Math.sin(time.secondsOfDay) * 1.5, 0),
			_modelMatrix2
		);
	});
