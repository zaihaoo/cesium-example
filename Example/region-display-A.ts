import {
	...
	KmlDataSource,
	Math,
	PolylineFocusMaterialProperty,
	Primitive,
	ScreenSpaceEventHandler,
	ScreenSpaceEventType,
	Viewer,
	WallBubblingMaterialAppearance,
	WallGeometry,
	WallGraduallyMaterialAppearance,
	AdministrationTextureMaterialProperty,
	Cartesian2
} from 'cesium';
...
let viewer: Viewer;
const entity_polyline_mapping: { [index: string]: any } = {};
const entity_label_mapping: { [index: string]: any } = {};
const entity_positions: { [index: string]: any } = {};

//记录上一个点击的entity
const last_obj: {
	polyline?: Entity;
	label?: Entity;
	move_region_id?: number;
	click_region_id?: number;
	focus_wall?: Primitive;
} = {};
export const example = async (viewer_param: Viewer) => {
	viewer = viewer_param;
	...
	handler.setInputAction(function (e: { endPosition: Cartesian2 }) {
	...
	handler.setInputAction(function (e: { position: Cartesian2 }) {
		...
		if (defined(pick) && defined(pick.id) && pick.id.name === 'region' && pick.id.id !== last_obj.click_region_id) {
			...
			if (pick.id.id in entity_positions) {
				...
				const max: number[] = [];
				let length_x = 0;
				entity_positions[pick.id.id].forEach((v: any, i: number) => {
					if (i !== 0) {
						const last = entity_positions[pick.id.id][i - 1];
						const current = v;
						length_x += window.Math.sqrt(
							window.Math.pow(current.x - last.x, 2) +
			...
			if (pick.id.id in entity_label_mapping) {
				last_obj.label = drawLabel(
					entity_label_mapping[pick.id.id].positions,
					entity_label_mapping[pick.id.id].name
				);
			}
		}
...
function xyz2latlng(xyz: Cartesian3) {
...
const loadProvincial = async () => {
	//加载省市
	const re: { primitive: Primitive[]; entity: Entity[] } = { primitive: [], entity: [] };
	...
	for (var i = 0; i < entities.length; i++) {
		...
		if (entity.polygon) {
			...
			let length_x = 0;
			polygon.positions.forEach((v: any, i: number) => {
				if (i !== 0) {
			...
			const encircle_line = EncircleLine(
				wall.boundingSphere.center,
				wall.boundingSphere.radius,
				new Color(0.0, 0.8, 0.8, 0.5)
			);
...
const loadCity = async () => {
	...
	for (var i = 0; i < entities.length; i++) {
		...
		if (entity.polygon) {
			...
			//画线
			const latlngs = xyzs.positions.flatMap((v: any) => {
				return xyz2latlng(v);
			});
...
function drawLine(latlngs: number[]) {
...
function drawLabel(polyPositions: Cartesian3[], name: any) {
...
function _createWellWall(position: any, min_heights: number[], max_heights: number[], width: number, height: number) {
