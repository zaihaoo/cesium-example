...
let _viewer: Viewer;
let _start: JulianDate;
let _stop: JulianDate;
export const example = (viewer: Viewer, gui: GUI) => {
	...
	helper.add(viewer.scene.globe.tileLoadProgressEvent, e => {
		if (e == 0) {
			...
			for (let i = 0; i < 4; i++) {
				const track = drawCircle(
					8500000 - window.Math.random() * 1000000,
					new SampledPositionProperty(),
					new ConstantPositionProperty(),
					new ConstantProperty(),
					window.Math.sin(window.Math.random() * 2) * 30,
					window.Math.cos(window.Math.random() * 2) * 30,
					window.Math.sin(window.Math.random() * 2) * 0.1,
					window.Math.cos(window.Math.random() * 2) * 0.1,
					100,
					40,
					Color.fromRandom(),
					modelB
...
const createCircle = (r: number, alphaX: number, alphaY: number, count: number, time: number) => {
	const pos = [];
	const times = [];
...
const drawCircle = (
	...
	_viewer.clock.onTick.addEventListener(v => {
		...
		const newpos = property.getValue(nowTime) as Cartesian3;
...
const updateProperty = (source: Cartesian3[], times: number[], property: SampledPositionProperty) => {
	// 取样位置 相当于一个集合
