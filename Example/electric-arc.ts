import { Color, Viewer, ElectricArc } from 'cesium';
...
let _viewer: Viewer;
let _obj: ElectricArc;
let _visible: Boolean = false;
export const example = (viewer: typeof Viewer.prototype, gui: typeof GUI.prototype) => {
	_viewer = viewer;
	_loadTestData();
	...
	folder
		...
		.name('位置')
		.onChange((v: string) => {
			_obj.position = { _position_$1: _position_$1, _position_$2: _position_$2 }[v] as [number, number];
		});
	folder
