...
const _loadWaterC = () => {
	...
	for (let i = 0, len = posArr.length; i < len; i++) {
		...
		const particleSystem = new ParticleSystem({
			...
			sizeInMeters: true,
			updateCallback: (p: { position: any; velocity: Cartesian3 }, dt: number) => {
...
function computeModelMatrix(position: Cartesian3) {
	let modelMatrix = Transforms.eastNorthUpToFixedFrame(position);
	return modelMatrix;
}

// 计算粒子发射器的位置姿态
function computeEmitterodelMatrix(rx: number, ry: number, rz: number, tx: number, ty: number, tz: number) {
