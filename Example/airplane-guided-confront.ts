...
const _animation = () => {
	...
	const computeModelMatrix = (entity: Entity, time: any) => {
	...
	function applyGravity(p: { position: any; velocity: Cartesian3 }, dt: number) {
	...
	function tick(clock: {
		currentTime: { dayNumber: number; secondsOfDay: number };
		stopTime: { dayNumber: number; secondsOfDay: number };
	}) {
		if (clock.currentTime.dayNumber >= stop.dayNumber && clock.currentTime.secondsOfDay > stop.secondsOfDay) {
			if (
				clock.currentTime.dayNumber >= clock.stopTime.dayNumber &&
				...
				clock.currentTime.dayNumber >= main_stop.dayNumber &&
				...
				clock.currentTime.dayNumber >= spread_stop.dayNumber &&
			...
			clock.currentTime.dayNumber >= guided_start.dayNumber &&
	 ...
	 */
	function createProperty(source: string | any[]) {
		// 取样位置 相当于一个集合
	...
	function applyAirGravity(p: { position: any; velocity: Cartesian3 }, dt: number) {
		// 计算每个粒子的向上向量（相对地心）
	...
	function applyGuidedGravity(p: { position: any; velocity: Cartesian3 }, dt: number) {
		// 计算每个粒子的向上向量（相对地心）
		...
		if (!guided_matrix) {
			...
			Cartesian3.normalize(
				Cartesian3.subtract(position, guided_position, new Cartesian3()),
				gravityGuidedScratch
			);
