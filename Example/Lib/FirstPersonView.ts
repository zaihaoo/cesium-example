import { Camera, Cartesian2, Math, Scene, ScreenSpaceEventHandler, ScreenSpaceEventType, defined } from 'cesium';

class FirstPersonView {
	private _camera: Camera;
	private _handler: ScreenSpaceEventHandler;
	private _mouseSensitivity: Cartesian2;
	private _heading: number;
	private _pitch: number;
	private _scene: Scene;
	private _screenSpaceCameraControllerBackup!: {
	...
	constructor(camera: Camera) {
		...
		this._camera = camera;
		this._scene = (this._camera as any)._scene;
	...
	_getFlagForKeyCode = (keyCode: string) => {
		switch (keyCode) {
			case 'w':
				return 'moveForward';
			case 's':
	...
	_keyboardEventHandler = (method: string) => {
		const flags = {
			moveForward: false,
			moveBackward: false,
			moveUp: false,
		...
		(this._scene.preUpdate as any)[method]((_: any) => {
			if (!this._working) return;
			const moveRate = 10;
	...
	_mouseMoveEvent = (event: { endPosition: Cartesian2; startPosition: Cartesian2 }) => {
		if (!this._working) return;
		const screenPositionDelta = Cartesian2.subtract(event.endPosition, event.startPosition, new Cartesian2());

		if (window.Math.abs(screenPositionDelta.x) > 200 || window.Math.abs(screenPositionDelta.y) > 200) return;
