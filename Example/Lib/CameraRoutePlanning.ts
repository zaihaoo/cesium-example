...
class CameraRoutePlanning {
	[x: string]: any;
	constructor(options: { clock: Clock; camera: Camera; loop: boolean; control: boolean }) {
	...
	timeSynchronization = (loop: boolean) => {
	...
	_onTick = (v: Clock) => {
		if (!this._property || !this._orienta || !this._clock.shouldAnimate) return;
		...
		this._control
			...
			: this._camera.setView({
					...
					orientation: {
						direction: dir,
						up: Cartesian3.cross(
							Cartesian3.cross(dir, Cartesian3.normalize(position, new Cartesian3()), new Cartesian3()),
							dir,
							new Cartesian3()
						),
					},
	...
	setPath = (controls: Cartesian3[], time: number = 60, count: number = 200) => {
		this._positions = [];
		const times: number[] = [];
		...
		for (let i = 0; i <= count; i++) {
			const cartesian3 = spline.evaluate(i / count) as Cartesian3;
			this._positions.push(cartesian3);
		}

		this._timePositions = [];
		this._positions.forEach((v: any, i: number) =>
			this._timePositions.push({ position: v, time: i * (time / this._positions.length) })
		);
