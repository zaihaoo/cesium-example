import { GeoJsonDataSource, Temperatrue, Viewer } from 'cesium';
...
let _viewer: Viewer;
let _obj: Temperatrue;
let _visible: Boolean = false;
let _datasources: GeoJsonDataSource[] | undefined;
export const example = async (viewer: Viewer, gui: GUI) => {
	...
	_obj = new Temperatrue(
		...
		(prev_primitive, primitive, _key, _value) => {
			prev_primitive &&
				prev_primitive.forEach((v: GeoJsonDataSource) => {
					_viewer.dataSources.remove(v);
				});

			primitive &&
				primitive.forEach((v: GeoJsonDataSource) => {
	...
	folder
		...
		.name('数据源')
		.onChange((v: string) => {
			_obj.dataset = { _dataset_$1: _dataset_$1, _dataset_$2: _dataset_$2 }[v] as {
				lat: number;
				lng: number;
				value: number;
			}[];
