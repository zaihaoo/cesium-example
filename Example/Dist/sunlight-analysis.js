import { Cartesian2, Cartesian3, Cartographic, 
// Cesium3DTileset,
CloudCollection, Color, Math, Matrix4, Model, Plane, Transforms, } from '../Lib/Cesium/index.js';
async function createModel(url, height) {
    ...
    // 	},
    // }
    const palaceTileset = _viewer.scene.primitives.add(await Model.fromGltfAsync({
        url: url,
...
export const cloud = (boundingbox, viewer) => {
    // viewer.entities.add({
    ...
    const cloudCollection = viewer.scene.primitives.add(new CloudCollection({
        noiseDetail: 16.0,
        noiseOffset: 0.5,
    }));
    ...
    const northAxis = Cartesian3.normalize(Cartesian3.subtract(Plane.projectPointOntoPlane(plane, Matrix4.multiplyByPoint(frame, new Cartesian3(0, 1, 0), new Cartesian3()), new Cartesian3()), position, new Cartesian3()), new Cartesian3());
    let i = 0;
    const clouds = [];
    const beginning_pos = [];
    while (i < count) {
        ...
        const nPos = Cartesian3.add(pos, Cartesian3.multiplyByScalar(northAxis, 50 * 3, new Cartesian3()), new Cartesian3());
        ...
        if (dot >= 0 &&
            ...
            const cloud = cloudCollection.add({
                ...
                maximumSize: new Cartesian3(cloudParameters.maximumSizeX, cloudParameters.maximumSizeY, cloudParameters.maximumSizeZ),
                color: Color.WHITE,
    ...
    viewer.clock.onTick.addEventListener(v => {
        const sunWC = viewer.scene._context.uniformState.sunPositionWC;
        ...
        clouds.forEach((c, i) => {
            ...
            Cartesian3.multiplyByScalar(northAxis, offset * radius, new Cartesian3()), new Cartesian3());
            const projectPos = Plane.projectPointOntoPlane(plane, nPos, new Cartesian3());
            c.position = nPos;
...
//# sourceMappingURL=sunlight-analysis.js.map