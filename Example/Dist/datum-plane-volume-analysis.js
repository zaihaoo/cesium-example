import { Cartesian3, Color, ColorGeometryInstanceAttribute, DatumPlaneAnalysis, EventHelper, GeometryInstance, GroundPrimitive, Material, MaterialAppearance, Math, Matrix3, Matrix4, PolygonGeometry, PolygonHierarchy, PolylineColorAppearance, PolylineGeometry, Primitive, PrimitiveCollection, Text3D, Transforms, } from '../Lib/Cesium/index.js';
...
const _updateVolume = () => {
    ...
    // 绘制参考面
    const datumPlane = Cartesian3.fromDegreesArrayHeights(_positions.flatMap(v => {
        return [v[0], v[1], _options.height];
    }));
    volume.instances.push(new GeometryInstance({
    ...
    _primitiveCollection.add(new Primitive({
        geometryInstances: volume.instances,
        appearance: new PolylineColorAppearance({
            translucent: false, //是否透明
        }),
    }));
    _primitiveCollection.add(new GroundPrimitive({
...
const _loadTestData = () => {
    ...
    _modelMatrix1 = Transforms.eastNorthUpToFixedFrame(Cartesian3.fromDegrees(86.84999390201867, 28.005400592303793, 7500));
    _modelMatrix2 = Transforms.eastNorthUpToFixedFrame(Cartesian3.fromDegrees(86.84439911685212, 27.94713359319177, 7500));
    _modelMatrix3 = Transforms.eastNorthUpToFixedFrame(Cartesian3.fromDegrees(86.9403064099265, 27.996796123763904, 8500));
    ...
    return BigeMapUtils.fontLoader('Assets/File/Typeface/KaiTi_Regular.json');
};
...
const _label = (volume) => {
    ...
    _fontGeo1 = BigeMapUtils.createText3DGeometryAttribute(`最低海拔 ${volume.minHeight} 米`, _typeface);
    _text3d1 = _primitiveCollection.add(new Text3D({
    ...
    _fontGeo2 = BigeMapUtils.createText3DGeometryAttribute(`基准面以上体积 ${volume.scoopVolume} 立方米`, _typeface);
    _text3d2 = _primitiveCollection.add(new Text3D({
    ...
    _fontGeo3 = BigeMapUtils.createText3DGeometryAttribute(`基准面以下体积 ${volume.fillVolume} 立方米`, _typeface);
    _text3d3 = _primitiveCollection.add(new Text3D({
...
//# sourceMappingURL=datum-plane-volume-analysis.js.map