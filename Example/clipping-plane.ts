...
let cartographic_positions: Cartographic[];
let cartesian3_positions: Cartesian3[];
let batch_index: number | undefined = undefined;
let _viewer: Viewer;
let subTriangleCartographicPositions: Cartographic[] = [];
let subTriangleGeometry: Geometry;
const entityCollection = new EntityCollection();
let textLabel: Entity;
...
const _loadTestData = async () => {
	...
	//polygon subdivision
	subTriangleGeometry = PolygonGeometry.createGeometry(
	...
	for (let i = 0; i < subTriangleGeometry.attributes.position.values.length; i += 3) {
		subTriangleCartographicPositions.push(
			Cartographic.fromCartesian(
				new Cartesian3(
					subTriangleGeometry.attributes.position.values[i],
					subTriangleGeometry.attributes.position.values[i + 1],
					subTriangleGeometry.attributes.position.values[i + 2]
				)
			)
		);
	}

	subTriangleCartographicPositions = await sampleTerrainMostDetailed(
		_viewer.terrainProvider,
		subTriangleCartographicPositions
	);
};
...
export const example = async (viewer: Viewer, gui: GUI) => {
	...
	options.draw = multiClippingPlane.draw.bind(multiClippingPlane, offset) as any;
	...
	folder
		...
		.onChange(v => {
			...
			entityCollection.values.forEach(entity => {
				...
				// 更新方量计算
				const polygon: PolygonHierarchy = entity.polygon.hierarchy!.getValue(new JulianDate());
...
const _computeCutVolume = async () => {
	...
	let p1: Cartesian3, p2: Cartesian3, p3: Cartesian3;
	let bottomP1: Cartesian3, bottomP2: Cartesian3, bottomP3: Cartesian3;
	...
	for (let i = 0; i < subTriangleGeometry.indices.length; i += 3) {
		...
		p1 = Cartesian3.fromRadians(
			subTriangleCartographicPositions[i0].longitude,
			subTriangleCartographicPositions[i0].latitude,
			height1
		);
		bottomP1 = Cartesian3.fromRadians(
			subTriangleCartographicPositions[i0].longitude,
			subTriangleCartographicPositions[i0].latitude,
			0
		);
		...
		p2 = Cartesian3.fromRadians(
			subTriangleCartographicPositions[i1].longitude,
			subTriangleCartographicPositions[i1].latitude,
			height2
		);
		bottomP2 = Cartesian3.fromRadians(
			subTriangleCartographicPositions[i1].longitude,
			subTriangleCartographicPositions[i1].latitude,
			0
		);
		...
		p3 = Cartesian3.fromRadians(
			subTriangleCartographicPositions[i2].longitude,
			subTriangleCartographicPositions[i2].latitude,
			height3
		);
		bottomP3 = Cartesian3.fromRadians(
			subTriangleCartographicPositions[i2].longitude,
			subTriangleCartographicPositions[i2].latitude,
			0
		);

		maxHeight = window.Math.max(
			maxHeight,
			subTriangleCartographicPositions[i0].height,
			subTriangleCartographicPositions[i1].height,
			subTriangleCartographicPositions[i2].height
		);
...
function computeAreaOfTriangle(pos1: Cartesian3, pos2: Cartesian3, pos3: Cartesian3) {
...
function computeCentroidOfPolygon(positions: string | any[]) {
	let x: number[] = [];
	let y: number[] = [];
