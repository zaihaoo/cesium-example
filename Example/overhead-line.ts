...
export const example = (viewer: Viewer, _: GUI) => {
	...
	for (let i = 0; i < data.length; i++) {
		...
		if (i === 0) {
			...
			const lPos = Cartesian3.fromDegrees(data[i - 1].lng, data[i - 1].lat, 0);
			const nPos = Cartesian3.fromDegrees(data[i + 1].lng, data[i + 1].lat, 0);
			const originEast = Cartesian3.normalize(
				Matrix3.getColumn(Transforms.eastNorthUpToFixedFrame(cPos), 0, new Cartesian3()),
				new Cartesian3()
			);
			const v = Cartesian3.normalize(
				Cartesian3.add(
					Cartesian3.normalize(Cartesian3.subtract(lPos, cPos, new Cartesian3()), new Cartesian3()),
					Cartesian3.normalize(Cartesian3.subtract(nPos, cPos, new Cartesian3()), new Cartesian3()),
					new Cartesian3()
				),
				new Cartesian3()
			);
	...
	for (let i = 0; i < data.length; i++) {
		if (i < data.length - 1) {
			data[i].points.map(v => {
				const startPoint = transformBias(data[i] as any, v as any),
					endPoint = transformBias(data[i + 1] as any, v as any);
...
function copy(bias: any) {
	return JSON.parse(JSON.stringify(bias));
}

function transformBias(
	point: { lng: number; lat: number; h: number; matrix: Matrix3 },
	...
	return Cartographic.fromCartesian(
		Matrix4.multiplyByPoint(matrix, new Cartesian3(bias.x, bias.y, bias.z), new Cartesian3())
	);
}

function getPath(start: Cartographic, end: Cartographic, sum: number, low: number) {
	const path: any = [];
	const half = sum / 2;
	const tmpH = {
		s: 0,
