...
let _viewer: Viewer;
let _tunnels: Tunnel[];
let _tunnelToCartesian3Positions: { [index: number]: Cartesian3[] } = {};
let options: { create: Function; clear: Function; height: number|undefined; scale: number; translucency: boolean };
	...
	folder
		...
		.onChange(v => {
			_tunnels.forEach((tunnel, i) => {
				const attributes = Tunnel.createAttributes(
					_tunnelToCartesian3Positions[i],
					arr => {
						return new BigeMapUtils.Vector3(...arr);
					},
					BigeMapUtils.createTunnelModelGeometryAttribute,
					BigeMapUtils.catmullRomCurve3,
					v
				);
	...
	_handler.setInputAction((event: { position: Cartesian2 }) => {
		...
		if (cartesian) {
			if (cartesian3_positions.length > 0 && cartesian.equals(last)) {
				...
				const tunnel = Tunnel.createTunnelModel(
					cartesian3_positions,
					arr => {
						return new BigeMapUtils.Vector3(...arr);
					},
					BigeMapUtils.createTunnelModelGeometryAttribute,
